/*---------------------------------------------------------------------------*
  ...
  $Log: res_print.c,v $
  Revision 1.21  03/02/2005 00:40:24  takano_makoto
  fix copyright header.

  Revision 1.20  02/28/2005 00:14:41  takano_makoto
  Warning measures for signed<->unsigned casts

  Revision 1.19  12/21/2004 08:27:07  nishida_kenji
  supports env map & proj map.

  Revision 1.18  10/06/2004 04:50:38  nishida_kenji
  workaround to NNSi_G3dFuncSbc_NODEMIX.

  Revision 1.17  09/17/2004 01:58:43  nishida_kenji
  REVISED

  Revision 1.16  09/15/2004 04:52:14  nishida_kenji
  small fix

  Revision 1.15  08/26/2004 07:50:05  nishida_kenji
  display NODEMIX command.

  Revision 1.14  07/19/2004 10:56:59  nishida_kenji
  (none)

  Revision 1.13  07/17/2004 13:00:25  nishida_kenji
  NODEDESC_BB -> BB
  NODEDESC_BBY -> BBY

  Revision 1.12  07/14/2004 11:29:09  nishida_kenji
  ...
  Revision 1.11  07/13/2004 05:17:36  nishida_kenji
  Add -s option for the .imd converter.

  Revision 1.8  07/06/2004 11:57:48  nishida_kenji
  Add NNS_G3dPrintDL(under construction)

  Revision 1.6  06/30/2004 05:30:00  nishida_kenji
  Workaround to the case that pos_scale is not 0.

  Revision 1.5  06/29/2004 02:08:46  nishida_kenji
...
//
// The contents of this file are only compiled for a debug build or
// by the converter.
//
...
/*---------------------------------------------------------------------------*
    ...
    Shows the file size, signature and version.    
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Outputs [Data block type] and data block size
...
/*---------------------------------------------------------------------------*
    ...
    Outputs the individual items of texture information. The data are output in the order:

    texture index, texture name, offset,
    width, height, format name, way of handling Pltt0, width of original data,

    height of original data.
 *---------------------------------------------------------------------------*/
void
                              ...
                              const NNSG3dResDictTexData* data)
{
    ...
    // Use value of GX_TEXSIZE_Sx
    width = 8 * (int)(pow(2, (double)((data->texImageParam >> 20) & 7)));
    // Use value of GX_TEXSIZE_Tx
    height = 8 * (int)(pow(2, (double)((data->texImageParam >> 23) & 7)));
...
/*---------------------------------------------------------------------------*
    ...
    Outputs the individual items of texture palette information.

    Index, palette name, offset inside block.
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Outputs the texture image block's information.
    The data includes palette information.
 *---------------------------------------------------------------------------*/
void
NNS_G3dPrintTex(const NNSG3dResTex* texImage)
{
    ...
    //
    // 4x4 and non-4x4 are divided up and displayed separately
    //
...
/*---------------------------------------------------------------------------*
    ...
    Displays a summary of the NNSG3dResMdlInfo structure
 *---------------------------------------------------------------------------*/
void
...
//
// Stores the locations of the matrix elements that become 0 upon existence of +1/-1 elements.
//
...
/*---------------------------------------------------------------------------*
    ...
    Outputs a summary of the SRT information for individual nodes
 *---------------------------------------------------------------------------*/
void
                        ...
                        u32 idx)
{
    ...
    tabPlus_();
    {
        ...
        if (!(srt->flag & NNS_G3D_SRTFLAG_ROT_ZERO))
        {
            ...
            {
                //
                // Interpret the compressed data and output
                //
                fx16* p = (fx16*)ptr;
...
/*---------------------------------------------------------------------------*
    ...
    Outputs the node information in a single model
 *---------------------------------------------------------------------------*/
void
...
//
// Subfunction group for NNS_G3dPrintSbc
//
...
/*---------------------------------------------------------------------------*
    ...
    Disassembles (?) and displays Sbc code
 *---------------------------------------------------------------------------*/
void
...
//
// Subfunction group for NNS_G3dPrintMatData
//

// Returns the texture's name if texture is being used with material ID (idx)
static const NNSG3dResName*
...
// Returns the palette's name if palette is being used with material ID (idx)
static const NNSG3dResName*
...
/*---------------------------------------------------------------------------*
    ...
    Output a summary of the individual materials
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Outputs the material information in a single model node
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Outputs the shape information in a single model
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Displays a summary of the model binary
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Displays a summary of the model set binary
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Displays a summary of the NSBMD binary
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Displays a summary of the NSBTX binary
 *---------------------------------------------------------------------------*/
void
...
/*---------------------------------------------------------------------------*
    ...
    Displays an appropriate summary of the binary file
 *---------------------------------------------------------------------------*/
void
...
//
// Other utilities (would it be best to divide into special file ?)
//
