/*---------------------------------------------------------------------------*
  ...
  $Log: env_system.h,v $
  Revision 1.15  10/13/2005 00:23:42  kitase_hirotake
  add ENV_SetResoruceSetArray

  Revision 1.14  08/31/2005 09:44:17  yada
  applied for multi env array

  Revision 1.13  08/23/2005 07:04:11  yada
  add ENV_SearchByPartialName()

  Revision 1.12  08/23/2005 01:27:46  yada
  rename ENV_GetBinaryAndLength() to ENV_GetBinaryAndSize()

  Revision 1.11  08/22/2005 12:05:26  yada
  consider for const

  Revision 1.10  08/22/2005 11:54:27  yada
  change get string / binary function

  Revision 1.9  08/22/2005 11:36:11  yada
  change get function

  Revision 1.8  08/22/2005 05:23:47  yada
  offset value available

  Revision 1.7  08/19/2005 08:16:59  yasu
  Revised so that installation is to the WiFi main tree using commondefs.add-ins.env

  Revision 1.6  08/18/2005 12:06:04  yada
  add ENV_SearchByType

  Revision 1.5  08/18/2005 08:24:38  yada
  add ENV_InitIter(), ENV_SearchByClass(), ENV_SearchByMember()

  Revision 1.4  08/18/2005 04:27:44  yada
  just fix comment

  Revision 1.3  08/17/2005 11:17:46  yada
  fix a little

  Revision 1.2  08/17/2005 10:57:12  yada
  add ENV_GetU64() / S64()

  Revision 1.1  08/17/2005 06:30:58  yada
  initial release
...
#define ENV_STRING(X) sizeof(X),    ENV_RESTYPE_STRING, (X)
#define ENV_BINARY(X) sizeof(X)-1,  ENV_RESTYPE_BINARY, (X)
