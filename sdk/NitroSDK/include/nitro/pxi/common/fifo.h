/*---------------------------------------------------------------------------*
  ...
  $Log: fifo.h,v $
  Revision 1.19.2.1  05/22/2006 02:22:24  okubata_ryoma
  PM synchronization function now runs even when interrupt is banned

  Revision 1.19  05/01/2006 01:14:54  yada
  add NITRO_FIFO_TAG_CTRDG_PHI

  Revision 1.18  01/18/2006 02:11:20  kitase_hirotake
  do-indent

  Revision 1.17  11/01/2005 01:17:36  okubata_ryoma
  Added PXI_FIFO_TAG_CTRDG_Ex

  Revision 1.16  02/28/2005 05:26:03  yosizaki
  do-indent.

  Revision 1.15  12/20/2004 00:42:41  terui
  Added TAG for WVR library

  Revision 1.14  09/27/2004 13:39:56  yada
  add PXI_FIFO_TAG_CARD

  Revision 1.13  09/13/2004 10:50:15  yada
  add PXI tag for CTRDG

  Revision 1.12  09/01/2004 04:24:35  yada
  add PXI_FIFO_TAG_OS tag

  Revision 1.11  07/13/2004 08:29:12  yada
  move PXI_IsArm7CallbackReady() and PXI_IsArm9CallbackReady()
  from pxi_fifo.c to this header as inline.

  Revision 1.10  05/14/2004 09:34:00  yasu
  add ARM9/ARM7 sync in PXI_Init

  Revision 1.9  04/21/2004 12:04:06  yasu
  change pxi tag id for FS

  Revision 1.8  04/15/2004 03:01:45  yasu
  add PXI_FIFO_TAG_FS

  Revision 1.7  03/30/2004 12:18:48  yasu
  change parameter of PXI_SetSendCallback

  Revision 1.6  03/30/2004 10:47:05  miya
  add send FIFO callback functions

  Revision 1.5  03/17/2004 02:51:35  yasu
  add some tags

  Revision 1.4  03/05/2004 06:43:36  ida
  add PXI_FIFO_TAG_SOUND

  Revision 1.3  03/05/2004 04:18:33  yasu
  PXI_MAX_FIFO_TAG=32 fixed

  Revision 1.2  03/04/2004 12:38:50  yasu
  change prefix, function name and macro

  Revision 1.1  03/04/2004 08:44:26  yasu
