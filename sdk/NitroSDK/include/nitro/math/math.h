/*---------------------------------------------------------------------------*
  ...
  $Log: math.h,v $
  Revision 1.13  01/18/2006 02:11:19  kitase_hirotake
  do-indent

  Revision 1.12  12/22/2005 06:04:01  yosizaki
  ...
  Revision 1.10  02/28/2005 05:26:24  yosizaki
  do-indent.
  ...
  Revision 1.7  12/16/2004 09:58:14  takano_makoto
  small fix

  Revision 1.6  12/15/2004 10:52:38  seiki_masashi
  Added MATH_ROUNDUP32,DOWN32

  Revision 1.5  12/15/2004 09:39:26  seiki_masashi
  Added MATH_ROUNDUP,DOWN

  Revision 1.4  12/15/2004 09:17:07  seiki_masashi
  Added MATH_CountPopulation

  Revision 1.3  12/15/2004 07:13:31  takano_makoto
  Added MATH_ABS, MATH_IAbs, MATH_CLAMP

  Revision 1.2  12/15/2004 00:34:25  seiki_masashi
  ...
  Revision 1.1  12/14/2004 10:52:20  seiki_masashi
  Added MATH genre
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Compares two arguments and returns the smaller one.
                ...
                form defining an unsigned operator.     Be careful about side effects, since each argument is subjected to multiple 
...
static u32 MATH_CountLeadingZerosInline(u32 x);
...
#if     defined(SDK_ARM9) && defined(SDK_CODE_ARM)
...
/*---------------------------------------------------------------------------*
  ...
  Description:  This is MATH_CountLeadingZeros under a different name
                Requests the number of 0 bits from the top, expressed as 32-bit binary.    
                In the ARM9 ARM code, this is 1 command.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Requests the integer part of log2(x) of the u32 argument x      
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Requests the number of 0 bits from the bottom, expressed as 32-bit binary.    
                Uses the MATH_CountLeadingZeros function.

  Arguments:    x             u32 value used in determining

  Returns:      The number of contiguous 0 bits from the bottom    
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*
  ...
  Description:  This is MATH_CountTrailingZeros under a different name
                Requests the number of 0 bits from the bottom, expressed as 32-bit binary.    
                Uses the MATH_CountLeadingZeros function.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Requests the bottom 1 bit, expressed as 32-bit binary    
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Another name for MATH_GetLeastSignificant
                Requests the bottom 1 bit, expressed as 32-bit binary    
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Requests the top 1 bit, expressed as 32-bit binary    
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Another name for MATH_GetMostSignificant
                Requests the top 1 bit, expressed as 32-bit binary    
