/*---------------------------------------------------------------------------*
  ...
  $Log: pm.c,v $
  Revision 1.52.2.1  05/22/2006 02:17:50  okubata_ryoma
  PM synchronization function now runs even when interrupt is banned

  Revision 1.52  03/28/2006 08:33:30  okubata_ryoma
  revised to continue waiting until LCD-OFF occurs

  Revision 1.51  03/28/2006 08:02:05  okubata_ryoma
  added SpinWait150ms

  Revision 1.50  03/23/2006 00:29:42  okubata_ryoma
  fixed bugs resulting from earlier specification changes

  Revision 1.49  03/13/2006 01:13:18  okubata_ryoma
  revised to always cut power in LCD_ON state

  Revision 1.48  01/18/2006 02:12:27  kitase_hirotake
  do-indent

  Revision 1.47  12/14/2005 02:05:56  yada
  fix the allowed range of command result from ARM7 via PXI for PMIC2

  Revision 1.46  07/28/2005 08:34:39  yada
  Fix PM_GoSleepMode().
  Sleep in LCD off and wakeup, GX status is like that GX_DispOff(). This is fixed.

  Revision 1.45  05/27/2005 02:00:28  yada
  refer accurately only 1 bit in checking battery

  Revision 1.44  02/28/2005 05:26:25  yosizaki
  do-indent.

  Revision 1.43  02/18/2005 07:30:00  yasu
  Corrected the return type of OS_DisableInterrupts.

  Revision 1.42  02/14/2005 01:03:22  yada
  just fix copyright year

  Revision 1.41  02/10/2005 12:38:17  yada
  fix rule in recovering from sleep mode.

  Revision 1.40  11/02/2004 09:46:58  yada
  just fix comment

  Revision 1.39  10/21/2004 07:56:44  yada
  fix recovery process after sleep mode.
  switch ON/OFF by GX_IsDispOn().

  Revision 1.38  10/08/2004 10:54:24  yada
  fix recovery process after sleep mode.
  that is, add GX_DispOn() and GXS_DispOn()

  Revision 1.37  10/06/2004 08:27:35  yada
  change around sleep callback

  Revision 1.36  10/05/2004 11:30:59  yada
  in sleeping, wait more one frame after turning back light off

  Revision 1.35  10/05/2004 04:13:39  yada
  support pre/post sleep mode callback

  Revision 1.34  10/04/2004 11:22:27  yada
  treat backlight status in sleeping, and recover

  Revision 1.33  09/30/2004 11:10:45  yada
  not change LED status in switching LCD power

  Revision 1.32  09/27/2004 13:45:45  yada
  consider for multi boot child

  Revision 1.31  09/16/2004 09:28:27  yada
  on power off, not turned on LCD after sleep

  Revision 1.30  09/15/2004 09:51:45  yada
  pulling out card in sleep causes power off if specified

  Revision 1.29  09/14/2004 09:16:13  yada
  fix a little

  Revision 1.28  09/13/2004 11:09:45  yada
  consider for cartridge interrupt

  Revision 1.27  09/09/2004 09:49:01  yada
  make LED not blink in power save mode

  Revision 1.26  08/26/2004 08:20:59  yada
  add PM_GetLEDPattern*

  Revision 1.25  08/24/2004 05:43:36  yada
  only change comment

  Revision 1.24  08/23/2004 08:06:16  yada
  add LED self blank

  Revision 1.23  08/16/2004 04:16:10  yada
  change argument of PMi_SetLCDPower().
  let PM_SetLCDPower() be sync ver.

  Revision 1.22  08/10/2004 10:53:23  yada
  only arrange spacing

  Revision 1.21  08/10/2004 10:50:48  yada
  LCD power interval becomes controlled with V-Blank counter

  Revision 1.20  08/10/2004 01:05:55  yada
  GX_PowerLCD -> GXi_PowerLCD

  Revision 1.19  08/09/2004 09:48:40  yada
  delete PM_SetSoundPower/SoundVolume, PM_GetSoundPower/SoundVolume
  (PM -> PMi)

  Revision 1.18  08/09/2004 05:00:42  yada
  add PM_GetSoundPower/SoundVolume/Amp/AmpGain

  Revision 1.17  08/09/2004 01:57:09  yada
  delete async function from PM_Get*

  Revision 1.16  08/07/2004 07:25:29  yada
  PM_CheckBattery -> PM_GetBattery

  Revision 1.15  08/07/2004 07:14:47  yada
  fix a little

  Revision 1.14  08/07/2004 07:01:45  yada
  PM_SetForcedPower -> PM_ForcedPowerOff

  Revision 1.13  08/06/2004 12:49:18  yada
  fix argument of PM_SetLCDPower() calling

  Revision 1.12  08/06/2004 12:39:50  yada
  delete power save mode proc

  Revision 1.11  08/05/2004 05:57:30  yada
  only arrange spacing

  Revision 1.10  08/05/2004 05:53:34  yada
  create functions about powerSaveMode

  Revision 1.9  08/04/2004 00:43:55  yada
  PM_SetBackLightTop/Bottom -> PM_SetBackLight

  Revision 1.8  08/03/2004 08:50:08  yada
  remove debug print

  Revision 1.7  08/03/2004 08:38:42  yada
  consider for turning LCD off

  Revision 1.6  08/02/2004 06:14:02  yada
  PM_SetLED, PM_WriteRegister, PM_ReadRegister : PM->PMi

  Revision 1.5  08/02/2004 02:30:51  yada
  comment out OS_Printf() line

  Revision 1.4  08/02/2004 02:23:48  yada
  stop->sleep

  Revision 1.3  08/02/2004 00:35:10  yada
  change some argument in pm api.

  Revision 1.2  07/31/2004 01:30:05  yada
  add PM_SetAmp, PM_SetAmpGain, PM_SetVDETVoltage

  Revision 1.1  07/29/2004 08:58:27  yada
  initial release
...
u32 PMi_SendSleepStart(u16 trigger, u16 keyIntrData)
{
    ...
    //---- turn LCD off
    (void)PMi_SetLCDPower(PM_LCD_POWER_OFF, PM_LED_BLINK_LOW, FALSE, FALSE);
...
u32 PM_ForceToPowerOffAsync(PMCallback callback, void *arg)
{
    ...
    if(LCDResult != PM_LCD_POWER_ON)
    {
        ...
        if(top != PM_BACKLIGHT_OFF) (void)PM_SetBackLight(PM_LCD_TOP, PM_BACKLIGHT_OFF);
        if(bottom != PM_BACKLIGHT_OFF) (void)PM_SetBackLight(PM_LCD_BOTTOM, PM_BACKLIGHT_OFF);
...
BOOL PMi_SetLCDPower(PMLCDPower sw, PMLEDStatus led, BOOL skip, BOOL isSync)
{
    ...
    case PM_LCD_POWER_ON:
        ...
        //     PMIC will be shut downed and never recover oneself.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  execute each callback registered to info list
