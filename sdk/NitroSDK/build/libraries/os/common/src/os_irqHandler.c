/*---------------------------------------------------------------------------*
  ...
  Revision 1.51  04/10/2006 06:04:01  yosizaki
  rollback to 1.49

  Revision 1.50  04/04/2006 08:36:03  yosizaki
  revisions related to the 8 byte alignment of the call stack

  Revision 1.49  01/18/2006 02:11:30  kitase_hirotake
  do-indent

  Revision 1.48  06/28/2005 09:49:19  yada
  applied for SDK_THREAD_INFINITY

  Revision 1.47  06/28/2005 04:47:24  yosizaki
  correct the code committed on Revision 1.46.

  Revision 1.46  06/28/2005 02:57:28  yosizaki
  avoid cpu bug. (add writeback-wait)

  Revision 1.45  02/28/2005 05:26:28  yosizaki
  do-indent.

  Revision 1.44  11/02/2004 10:25:09  yada
  just fix comment

  Revision 1.43  07/10/2004 06:11:24 AM  yasu
  Support to select a section 'ITCM/DTCM/WRAM'

  Revision 1.42  07/02/2004 08:46:00  yada
  section name Dtcm->DTCM, Itcm->ITCM

  Revision 1.41  07/01/2004 12:57:47  yada
  send IrqHandler() into .itcm and .dtcm section
  arrange function description

  Revision 1.40  06/15/2004 12:35:05  yada
  consider for saving/loading CPContext in switching thread in IrqHandler

  Revision 1.39  05/25/2004 10:44:58  yada
  when IE&IF==0 in irqHandler, leave without IF=0

  Revision 1.38  05/24/2004 04:48:29  yada
  OS_WaitIntr() -> OS_WaitInterrupt()

  Revision 1.37  05/20/2004 04:29:56  yada
  consider for SDK_NO_THREAD

  Revision 1.36  05/18/2004 12:16:09 AM  yasu
  re-support OSi_IrqThreadQueue in irqHandler

  Revision 1.35  05/18/2004 11:59:44 AM  yasu
  add IRQ level counter 'irqDepth' in OSThreadInfo

  Revision 1.34  05/17/2004 06:47:35  yada
  in OS_IrqHandler(), if IME=0 then return.

  Revision 1.33  05/10/2004 12:37:12  yada
  forbid interrupt while clearing checkflag

  Revision 1.32  05/10/2004 11:24:52  yada
  SVC_WaitIntr() -> OS_WaitIntr()

  Revision 1.31  05/06/2004 11:14:48  takano_makoto
  fix a bug when SDK_NO_THREAD defined.

  Revision 1.30  05/01/2004 05:47:31  yada
  OS_WaitInterrupt->OS_WaitIrq, OS_WaitAnyInterrupt->OS_WaitAnyIrq

  Revision 1.29  04/30/2004 08:11:06  yada
  add OS_WaitAnyInterrupt

  Revision 1.28  04/30/2004 02:19:17  yada
  add OS_WaitInterrupt().
  change irqHandler to see irqThreadQueue

  Revision 1.27  04/15/2004 12:50:23  yada
  divide OS_IrqHandler to 2 function

  Revision 1.26  04/14/2004 04:13:31 AM  yasu
  Fix irqHandler when IE & IF == 0 && NO_THREAD

  Revision 1.25  04/08/2004 02:36:40  yada
  Made it to exit as IF=0 when IE&IF==0 inside the Interrupt handler.

  Revision 1.24  03/31/2004 09:25:14  yada
  add thread switch callback function

  Revision 1.23  03/30/2004 06:09:29  yada
  Changed ldconst and lda to the description that uses ldr =.

  Revision 1.22  02/26/2004 09:44:12  yada
  Made changes so that the thread manages the priority list,
  Revisions that related to that.

  Revision 1.21  02/16/2004 00:17:55  yada
  only added comment

  Revision 1.20  02/13/2004 01:15:32  yada
  SPOS->OS

  Revision 1.19  02/12/2004 13:38:42 AM  yasu
  change directory of ioreg_OS.h

  Revision 1.18  02/12/2004 11:08:30  yada
  Worked on distribution of processing for ARM9/ARM7.

  Revision 1.17  02/05/2004 07:09:02 AM  yasu
  change SDK prefix iris -> nitro

  Revision 1.16  01/18/2004 02:25:37  yada
  Reformed indents, etc.

  Revision 1.15  12/25/2003 07:31:26  yada
  Changes as per unified type rules

  Revision 1.14  12/22/2003 13:38:56 AM  yasu
  Lifted the prohibition on ARCH5 code

  Revision 1.13  12/17/2003 08:20:20 AM  yasu
  Made the code to be shared with ARM7

  Revision 1.12  12/17/2003 06:27:44 AM  yasu
  Added code for ARCH4

  Revision 1.11  12/16/2003 06:42:30 AM  yasu
  In order to support ISDebugger, prohibited FIQ at the time the thread switches

  Revision 1.10  12/16/2003 06:01:03 AM  yasu
  Changed the IRQ vector calculation process

  Revision 1.9  12/16/2003 00:17:22  yada
  Changed as per collection of thread information

  Revision 1.8  12/12/2003 04:40:19  yada
  Corrections in line with changes to processor mode definitions

  Revision 1.7  12/08/2003 12:16:45  yada
  Changed the file to place in an include statement from MemoryMap.h to mmap_global.h

  Revision 1.6  12/08/2003 10:11:00 AM  yasu
  Deleted the unnecessary define

  Revision 1.5  12/08/2003 01:25:05 AM  yasu
  Added SP_svc to 'context'

  Revision 1.4  12/01/2003 09:51:04 AM  yasu
  Value saved in context changed: PC->PC+4

  Revision 1.3  11/30/2003 05:10:42 AM  yasu
  Made it possible to call thread-related functions from interrupt service

  Revision 1.2  11/29/2003 04:36:30 AM  yasu
  Revised names

  Revision 1.1  11/29/2003 03:58:56 AM  yasu
  Changed file name  osInterruptBase.c -> os_irqhandler.c

  Revision 1.7  11/28/2003 12:01:57 AM  yasu
  Added OSi_IRQHandler_Begin_Label/End_Label

  Revision 1.6  11/27/2003 13:39:14 AM  yasu
  Fixed a bug in the jump table

  Revision 1.5  11/27/2003 10:14:18 AM  yasu
  Changed ICE-related measures SDK_BB -> SDK_BB_ICE

  Revision 1.4  11/27/2003 07:34:55 AM  yasu
  Added support for the Context Switch after the interrupt has finished
  Added support for the change of the VBlank to 30Hz on the BreadBoard

  Revision 1.2  11/10/2003 02:33:19 AM  yasu
  Added  'interrupt table'

  Revision 1.1  11/06/2003 13:39:01 AM  yasu
  osInterruptBase.s was changed to .c on the inline assembler
  $NoKeywords: $
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Interrupt Branch Processing (Uses the OS_InterruptTable)
