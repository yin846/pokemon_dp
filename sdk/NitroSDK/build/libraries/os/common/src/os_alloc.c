/*---------------------------------------------------------------------------*
  ...
  $Log: os_alloc.c,v $
  Revision 1.29  03/27/2006 05:35:35  kitase_hirotake
  revised the OS_CheckHeap function to allow OS_CURRENT_HEAP_HANDLE to be used

  Revision 1.28  01/18/2006 02:11:30  kitase_hirotake
  do-indent

  Revision 1.27  10/07/2005 06:23:25  yada
  add OS_GetTotalOccupiedSize()

  Revision 1.26  02/28/2005 05:26:27  yosizaki
  do-indent.

  Revision 1.25  02/18/2005 08:51:35 AM  yasu
  Changed year of creation.

  Revision 1.24  02/18/2005 06:19:32 AM  yasu
  Corrected the return type of OS_DisableInterrupts.

  Revision 1.23  11/08/2004 00:49:29  yada
  fix assert

  Revision 1.22  11/05/2004 12:34:04  yada
  OS_GetAllocatedSize() -> OS_GetTotalAllocSize()

  Revision 1.21  11/05/2004 10:35:23  yada
  add OS_FreeAllToHeap

  Revision 1.20  11/05/2004 06:31:34  yada
  Change type of OS_CheckHeap(): long -> s32 .
  OS_GetAllocatedSize() - OS_GetTotalAllocSize()
  OS_GetMaxFreeSize(), OS_ClearHeap()

  Revision 1.19  10/25/2004 10:59:56 AM  yasu
  Support SDK_NO_MESSAGE

  Revision 1.18  09/02/2004 06:18:58  yada
  fix bug that OS_CheckHeap() don't restore interrupt status

  Revision 1.17  08/31/2004 01:56:59  yada
  fix ASSERT message

  Revision 1.16  08/31/2004 01:06:36  yada
  add OS_ClearAlloc

  Revision 1.15  07/19/2004 01:43:26  yada
  only fix comment

  Revision 1.14  07/02/2004 00:04:05  yada
  fix a small bug

  Revision 1.13  07/01/2004 09:58:23  yada
  let functions in os_alloc.c be thread safe

  Revision 1.12  06/08/2004 00:24:01  yada
  add '#pragma unused()' for FINALROM

  Revision 1.11  03/08/2004 11:51:48  yada
  Added check when attempting to allocate from heap without using InitAlloc.

  Revision 1.10  02/13/2004 10:38:26  yada
  Changed from 6 to 9 arenas.

  Revision 1.9  02/12/2004 11:08:30  yada
  Worked on distribution of processing for ARM9/ARM7.

  Revision 1.8  02/05/2004 07:09:02 AM  yasu
  change SDK prefix iris -> nitro

  Revision 1.7  01/18/2004 01:24:32  yada
  Corrected number of default values for HeapInfo.

  Revision 1.6  01/07/2004 08:20:53  yada
  Made it so free/alloc list pointers are cleared when OS_DestroyHeap() is called.

  Revision 1.5  01/07/2004 02:51:44  yada
  Changed the value for the size (excluding the header) when calling OS_VisitAllocated.

  Revision 1.4  01/07/2004 00:26:10  yada
  Made corrections.

  Revision 1.3  01/07/2004 00:21:13  yada
  Take as current specification when the heap handle is negative when using alloc and free.

  Revision 1.2  01/05/2004 08:23:15  yada
  Changed so that each arena possesses heap information.

  Revision 1.1  01/05/2004 04:29:38  yada
  Initial release.
    ...
    for (i = 0; i < heapInfo->numHeaps; i++)
    {
        ...
        for (cell = hd->free; cell; cell = cell->next)
        {
                ...
                InRange((char *)cellEnd, start, (char *)end + MINOBJSIZE))
            {
                ...
                //---- cell is completely overlapped. Just extract this block
...
{                                                                        \
    if ( !(exp) )                                                        \
    {                                                                    \
        goto exit_OS_CheckHeap; /* goto is not beautiful, but less code*/ \
    }                                                                    \
} while (0)
#endif
