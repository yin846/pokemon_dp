/*---------------------------------------------------------------------------*
  ...
  Revision 1.13  01/18/2006 02:12:27  kitase_hirotake
  do-indent

  Revision 1.12  03/04/2005 12:43:00  seiki_masashi
  ASSERT conditions were too strict. Revised.

  Revision 1.11  02/28/2005 05:26:29  yosizaki
  do-indent.

  Revision 1.10  02/18/2005 07:10:49  seiki_masashi
  Revised Copyright notation

  Revision 1.9  02/18/2005 07:09:52  seiki_masashi
  warning measures

  Revision 1.8  11/04/2004 07:57:46  seiki_masashi
  Revised regularize to normalize

  Revision 1.7  08/31/2004 02:04:05  seiki_masashi
  Corrected comments

  Revision 1.6  07/19/2004 06:52:06  seiki_masashi
  Arrangement of identifier names

  Revision 1.5  07/09/2004 00:12:36  seiki_masashi
  Minor fixes

  Revision 1.4  07/05/2004 13:19:25  seiki_masashi
  Changed macro names
  Reconsidered criteria for angle score

  Revision 1.3  07/05/2004 05:45:14  seiki_masashi
  Made distance calculations more detailed

  Revision 1.2  07/05/2004 02:41:01  seiki_masashi
  Reduced the number of ratio calculations
  Added use of FX_DivAsync
  ...
  Revision 1.5  06/29/2004 02:14:20  seiki_masashi
  #if NITRO_DEBUG -> #ifdef SDK_DEBUG
  Implemented correction based on correction value

  Revision 1.4  06/28/2004 11:11:36  seiki_masashi
  Added regularizeSize to prototype patterns list    
  Adjusted parameter types    

  Revision 1.3  06/25/2004 13:32:11  seiki_masashi
  Addition of recognition algorithm "Standard"

  Revision 1.2  06/25/2004 03:02:55  seiki_masashi
  Deleted bufferSize specification from various functions
  Addition of GetRecognitionBufferSize*

  Revision 1.1  06/24/2004 08:09:19  seiki_masashi
...
// (up)The speed increases as it is raised.     Precision does not drop that much.
...
#define angleScores_(x,y) (*(angleScores + (x) * (maxPointCount+1) + (y)))
    ...
    // Calculate the degree of stroke similarity with DP matching
    int     iInput, iProto;
    ...
    *score = 0;
    ...
    // Segments with lengths that differ to a great extent have a similarity of 0. Computation is halted.    
    ...
    // Calculate in advance angleScores[][] used for scoring angles     
    ...
    // Use DP matching to find matching points between strokes
    sumScore_(0, 0) =
    ...
    // Similarity calculation phase
    {
        ...
        // First, extract pairs that clearly match
        for (iInput = 0; iInput < inputSize; iInput++)
        {
            inputPair[iInput] = -1;
        ...
        for (iInput = 0; iInput < inputSize; iInput++)
        {
            ...
            if (pair >= 0)
            {
                // See if mate has mutual feelings     
...
//#define PRC_DEBUG_RECOGNIZE_DEEPLY
        ...
        for (iMatch = 0; iMatch < nMatches - 1; iMatch++)
        {
            ...
            {
                // Initially, duplication is possible with [0, 0] [0, 0]     
                continue;
            }
